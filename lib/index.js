...
class EngineEntrypointManager {
    ...
    set (address, instance, private) {
        var o = this.get(!!private);
        setValue(o, address, instance);
    }
    delete (address, private) {
        this.set(address, undefined, private);
    }
    get (address, private) {
        if (typeof address === "boolean") {
            return deffy(this.data[address ? "private": "public"], {});
        }
        if (!address) { return this.data; }
        private = !!private;
        return findValue(this.get(private), address);
    }
}
